  ...
  "changeUserPassword": function(options) {
    function ChangePassword(callback) {
      if(!options || !options.username || !options.password || !options.verifyCode) {
        log("changeUserPassword: password options not completely", options);
        ...
        if(codeLegal) {
          ...
          if(user['services']['password']['bcrypt'] === hashPassword(password)) {
            ...
            Meteor.users.update({
              ...
              if(err) {
                log("changeUserPassword: update user [" + username +  "] password error", err);
                callback(err, {status: 0, message: "修改密码失败！"});
              } else {
                log("changeUserPassword: update user [" + username +  "] password succeed");
                callback(null, {status: 1, message: "修改密码成功"});
              }
            })            
          }
        } else {
         log("changeUserPassword: code not exists or time out");
